...
export async function GET() {
  try {
    ...
    const lines = content.split('\n'); // works for both \r\n and \n if we just use string operations carefully, or better:
    ...
    if (endOfTopper === 0) {
      return new Response("Could not find topper section", { status: 200 });
    }
...
`;
    ...
    // We can join with the original newline char, but \n is fine.
    fs.writeFileSync(filePath, newFileLines.join('\n'), 'utf-8');
    
    return new Response("Updated successfully", { status: 200 });
  } catch (error) {
    return new Response(error.toString(), { status: 200 });
  }
}
